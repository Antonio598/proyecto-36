...
// Reminder windows: { reminderNumber, msBeforeAppointment, toleranceMs }
...
export async function POST(request: Request) {
  ...
  if (secret) {
    const authHeader = request.headers.get('x-reminders-secret');
  ...
  for (const window of REMINDER_WINDOWS) {
    ...
    for (const appt of appointments) {
      ...
      try {
        ...
        if (!res.ok) {
          errors.push(`appt ${appt.id} reminder ${window.reminderNumber}: webhook returned ${res.status}`);
          continue;
        }
